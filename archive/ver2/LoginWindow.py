from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QLineEdit, QInputDialog, QMessageBox
...
class LoginWindow(QWidget):
    ...
    def show_menu_after_welcome(self):
        ...
        self.menu_window = MenuWindow(self.db, self.pwm)
        self.menu_window.show()
        self.close()

    # In MenuWindow.py (example)
    def change_password(self):
        new_pass, ok = QInputDialog.getText(self, "Change Password",
                                            "Enter new password (leave blank for no password):",
                                            QLineEdit.Normal)
        if ok:
            self.db_password_manager.set_password(new_pass)
